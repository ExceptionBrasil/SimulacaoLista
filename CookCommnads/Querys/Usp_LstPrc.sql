 ...
 ** Data : 03/09/2018                
 ** Cliente: Colorobbia                 
 ** Descrição: Procedure para gerar a Lista de Preço               
 */                
 ...
 --ALTERAÇÃO PARA PEGAR A ÚLTIMA REVISAO DA PRODUCAO             
 SET @RevisãoAtual=(SELECT case B1_REVATU when '' then null else B1_REVATU end   FROM SB1010 WHERE B1_COD=@NextProduto AND B1_FILIAL=@Filial AND B1_REVATU<>'' AND D_E_L_E_T_='') 
   --SET @RevisãoAtual=( select TOP 1 C2_REVISAO from SC2010  WHERE  D_E_L_E_T_=''  AND C2_PRODUTO=@NextProduto AND C2_FILIAL=@Filial   ORDER BY C2_EMISSAO DESC )
 ...
 if  @RevisãoAtual is null               
 BEGIN              
	SET @RevisãoAtual=(SELECT MAX(G1_REVFIM) FROM SG1010  WHERE G1_COD=@NextProduto AND D_E_L_E_T_='' AND G1_FILIAL=@Filial and G1_REVFIM NOT IN (''))              
 END         
 ...
 ------------------------------------------------------------             
  ...
  Begin              
   ...
   Begin                
		   ...
		   -- SET @RevisãoAtual=( select TOP 1 C2_REVISAO from SC2010  WHERE  D_E_L_E_T_=''  AND C2_PRODUTO=@Componente AND C2_FILIAL=@FILIAL   ORDER BY C2_EMISSAO DESC )          
			...
			Begin                 
			 ...
			 WHERE                 
			  ...
			  AND B1_REVATU<>''                 
			  AND D_E_L_E_T_=''                 
			  --SET @RevisãoAtual=( select TOP 1 C2_REVISAO from SC2010  WHERE  D_E_L_E_T_=''  AND C2_PRODUTO=@Produto AND C2_FILIAL=@FILIAL   ORDER BY C2_EMISSAO DESC )                          
